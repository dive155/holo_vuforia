...
namespace HoloToolkit.Unity.InputModule
{
    ...
    /// </summary>
    public class HandDraggable : MonoBehaviour, IFocusable, IInputHandler, ISourceStateHandler, IInputClickHandler
    {
        /// <summary>
        ...
        private void UpdateDragging()
        {
			...
			Vector3 nextPos = Vector3.Lerp(HostTransform.position, draggingPosition, PositionLerpSpeed); //+ cameraTransform.TransformDirection(objRefGrabPoint), PositionLerpSpeed);
			if (!Physics.Linecast(lastPos, nextPos))
			{
            	HostTransform.position = nextPos;
			}
			//this.GetComponent<Rigidbody>().MovePosition(Vector3.Lerp(HostTransform.position, draggingPosition + cameraTransform.TransformDirection(objRefGrabPoint), PositionLerpSpeed));
		...
		void OnCollisionStay(Collision col)
		{
			...
			if (other.GetComponent<Collider> ().bounds.Intersects (this.GetComponent<Collider> ().bounds)) 
			{
				other.transform.Translate ((transform.position - col.contacts [0].point).normalized * 0.1f);
				StopDragging ();
			}
		}*/
    }
